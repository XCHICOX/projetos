...
def enviar_whatsapp():
    ...
    whatsapp_url = f"https://api.whatsapp.com/send?phone=55{numero_whatsapp}&text=Olá! Em anexo está o orçamento estamos a disposição."
...
abas.add(aba_enviar, text='Enviar')  # Adiciona a aba "Enviar"
abas.pack(expand=1, fill='both')

# Aba Dados e Serviços
...
tk.Label(aba_dados_e_servicos, text="E-mail:").grid(row=3, column=0, padx=2, pady=2, sticky='w')
...
entry_email = tk.Entry(aba_dados_e_servicos, width=50)
...
tk.Label(aba_enviar, text="Selecionar PDF:").pack(pady=5)

listbox_pdfs = tk.Listbox(aba_enviar, width=50, height=10)
listbox_pdfs.pack(pady=5)

tk.Button(aba_enviar, text="Enviar E-mail", command=enviar_pdf_por_email).pack(pady=5)
tk.Button(aba_enviar, text="Enviar Mensagem WhatsApp", command=enviar_whatsapp).pack(pady=5)

# Carrega os PDFs na Listbox ao iniciar
carregar_docx()

# Botão para salvar a ordem de serviço
tk.Button(aba_dados_e_servicos, text="Salvar Ordem de Serviço", command=salvar_ordem).grid(row=6, column=0, columnspan=2, pady=5)

# Campo para número da OS para abrir
tk.Label(aba_dados_e_servicos, text="Número da OS para abrir:").grid(row=7, column=0, padx=2, pady=5, sticky='w')
...
tk.Button(aba_dados_e_servicos, text="Abrir Arquivo pelo Número", command=abrir_arquivo_pelo_numero).grid(row=8, column=0, columnspan=2, pady=5)

# Botão para abrir o arquivo salvo
arquivo_salvo = [None]  # Lista para manter referência ao arquivo salvo
botao_abrir = tk.Button(aba_dados_e_servicos, text="Abrir Arquivo Salvo", command=abrir_arquivo, state=tk.DISABLED)
botao_abrir.grid(row=9, column=0, columnspan=2, pady=5, sticky='w')
