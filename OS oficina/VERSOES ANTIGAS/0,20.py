...
def carregar_icone(nome_icone, tamanho=(16, 16)):
    caminho_icone = icone_pasta / nome_icone
    imagem = Image.open(caminho_icone)
    imagem = imagem.resize(tamanho, Image.LANCZOS)  # Redimensiona a imagem
    return ImageTk.PhotoImage(imagem)
...
tk.Button(aba_dados_e_servicos, text="Salvar Ordem de Serviço", image=icone_salvar, command=salvar_ordem).grid(row=6, column=0, columnspan=2, pady=5)
...
tk.Button(aba_dados_e_servicos, text="Abrir Arquivo pelo Número", image=icone_abrir_numero, command=abrir_arquivo_pelo_numero).grid(row=8, column=0, columnspan=2, pady=5)
