...
# Função para enviar e-mail (exemplo básico)
def enviar_email():
    mensagem = "Esta é uma mensagem de exemplo para enviar por e-mail."
    messagebox.showinfo("Enviar E-mail", mensagem)

# Função para enviar mensagem via WhatsApp
def enviar_whatsapp():
    telefone = entry_telefone.get()
    mensagem = f"Olá, {entry_nome_cliente.get()}. Sua ordem de serviço foi salva com sucesso!"
...
# Aba Enviar
tk.Button(aba_enviar, text="Enviar E-mail", command=enviar_email).pack(pady=10)
tk.Button(aba_enviar, text="Enviar WhatsApp", command=enviar_whatsapp).pack(pady=10)
