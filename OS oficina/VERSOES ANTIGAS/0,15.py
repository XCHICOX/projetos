...
for i in range(num_linhas):
    ...
    total_entry = tk.Entry(frame_pecas, width=6, state='readonly', justify='left')
...
tk.Button(root, text="Salvar Ordem de Serviço", command=salvar_ordem).pack(pady=5)

# Campo para número da OS para abrir
tk.Label(root, text="Número da OS para abrir:").pack(pady=5)
entry_numero_os = tk.Entry(root, width=10)
entry_numero_os.pack(pady=2)

# Botão para abrir arquivo pelo número
tk.Button(root, text="Abrir Arquivo pelo Número", command=abrir_arquivo_pelo_numero).pack(pady=5)
