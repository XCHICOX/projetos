...
class VeiculosApp(QMainWindow):
    ...
    def setup_manutencoes_tab(self):
        ...
        # Layout for the maintenances tab
        self.tab_manutencoes_layout = QVBoxLayout(self.tab_manutencoes)

        # Search bar
        self.search_bar_manutencoes = QLineEdit()
        self.search_bar_manutencoes.setPlaceholderText('Buscar por placa ou data...')
        self.search_bar_manutencoes.textChanged.connect(self.filtrar_manutencoes)
        ...
        self.tab_manutencoes_layout.addWidget(self.search_bar_manutencoes)
    ...
    def filtrar_manutencoes(self):
        search_text = self.search_bar_manutencoes.text().lower()
        self.listaManutencoes.clear()
    ...
    def adicionar_veiculo(self):
        ...
        def save():
            if placa.text() and modelo.text() and ano.text():
                veiculo = {
                    'placa': placa.text(),
                    'modelo': modelo.text(),
                    'ano': ano.text()
    ...
    def adicionar_manutencao(self):
        ...
        def save():
            if data.text() and placa_combo.currentText() and informacoes.text():
                manutencao = {
                    'data': data.text(),
    ...
    def editar_manutencao(self):
        ...
        if selected_item:
            ...
            data, placa, informacoes = self.extract_manutencao_info(manutencao_info)
            ...
            def save():
                if data_edit.text() and placa_edit.text() and informacoes_edit.text():
                    manutencao = {
                        'data': data_edit.text(),
                        'placa': placa_edit.text(),
                    ...
                    for i, m in enumerate(self.manutencoes):
                        if m['placa'] == placa and m['data'] == data:  # Assuming Placa and Data are unique
    ...
    def excluir_manutencao(self):
        ...
        if selected_item:
            manutencao_info = selected_item.text()
            data, placa, _ = self.extract_manutencao_info(manutencao_info)
