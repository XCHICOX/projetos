...
class VeiculosApp(QMainWindow):
    ...
    def editar_manutencao(self):
        ...
        if selected_item:
            ...
            def save():
                if data_edit.text() and placa_edit.text() and informacoes_edit.text() and mecanico_edit.text() and valor_edit.text():
                    try:
                        float(valor_edit.text())  # Verifica se o valor é um número
                    except ValueError:
                        QMessageBox.warning(self, 'Erro', 'O valor deve ser numérico!')
                        return
                    ...
                    for i, m in enumerate(self.manutencoes):
                        if m['data'] == data and m['placa'] == placa:  # Assuming data and placa uniquely identify a manutenção
