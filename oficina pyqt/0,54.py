...
class VeiculosApp(QMainWindow):
    ...
    def listar_clientes(self):
        ...
        for cliente in self.clientes:
            ...
            self.listaClientes.addItem(f"Nome: {cliente['nome']} - RG: {cliente['rg']} - Endereço: {cliente['endereco']} - Telefone: {cliente['telefone']} - WhatsApp: {cliente['whatsapp']} - Por: {usuario_cadastro}")
    ...
    def listar_veiculos(self):
        ...
        for veiculo in self.veiculos:
            if 'usuario_cadastro' in veiculo:
                ...
                usuario_cadastro = 'Desconhecido'
            self.listaVeiculos.addItem(f"Placa: {veiculo['placa']} - Modelo: {veiculo['modelo']} - Ano: {veiculo['ano']} - Por: {usuario_cadastro}")
        ...
        def save():
            if data.text() and placa_combo.currentText() and informacoes.text() and mecanico.text() and valor.text():
                ...
                manutencao = {
                    ...
                    'valor': valor.text(),
                    'usuario_cadastro': self.logged_in_username  # Adiciona o usuário que cadastrou a manutenção
                }
    ...
    def listar_manutencoes(self):
        ...
        for manutencao in self.manutencoes:
            ...
            usuario_cadastro = manutencao.get('usuario_cadastro', 'Desconhecido')
            self.listaManutencoes.addItem(f"Data: {data} - Veículo: {placa} - Informações: {informacoes} - Mecânico: {mecanico} - Valor: {valor} - Por: {usuario_cadastro}")
